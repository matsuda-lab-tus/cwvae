...
if __name__ == "__main__":
    ...
    checkpoint.restore(model, os.path.basename(os.path.normpath(args.logdir)))
    ...
    with torch.no_grad():  # No need to compute gradients for evaluation
        for i_ex in range(args.num_examples):
            try:
                gts = np.tile(
                    get_single_batch(val_data_batch),
                    [args.num_samples, 1, 1, 1, 1],
                )
                gts_tensor = torch.tensor(gts, dtype=torch.float32).to(device)

                preds = model_components["open_loop_obs_decoded"]["prior_multistep"](
                    gts_tensor
                )

                # Move data back to CPU and convert to NumPy for metrics computation
                preds_np = preds.cpu().numpy()

                # Computing metrics.
                ssim, psnr = tools.compute_metrics(gts[:, args.open_loop_ctx:], preds_np)
