...
)  # SSIM（構造類似度）を計算するための関数
from skimage.metrics import (
    peak_signal_noise_ratio as psnr_metric,
)  # PSNR（ピーク信号対雑音比）を計算するための関数
...
def exp_name(cfg, model_dir_prefix=None):
    ...
    """
    exp_name = (
        f"{cfg.dataset}_cwvae_{cfg.cell_type.lower()}"  # データセット名とモデルタイプ
    )
...
def validate_config(cfg):
    ...
    ), f"Incompatible channels = {cfg.channels} found in config."  # チャンネル数を確認
    assert (
    ...
    assert (
        cfg.datadir is not None
    ), "data root directory cannot be None."  # データディレクトリがNoneでないことを確認
    assert (
        cfg.logdir is not None
    ), "log root directory cannot be None."  # ログディレクトリがNoneでないことを確認
...
def read_configs(config_path, base_config_path=None, **kwargs):
    ...
    def torch_device_constructor(loader, node):
        ...
        # どちらでもない場合はエラーを投げる
        raise ValueError(
            f"Invalid format for torch.device: {loader.construct_object(node)}"
        )
    ...
    if base_config_path is not None:  # 基本設定ファイルが指定されている場合
        base_config = yaml.safe_load(
            Path(base_config_path).read_text()
        )  # 基本設定を読み込む
        config = base_config.copy()  # 基本設定をコピー
        config.update(
            yaml.load(Path(config_path).read_text(), Loader=TorchDeviceLoader)
        )  # 新しい設定で更新
    else:
...
def scan(cell, inputs, use_obs, initial):
    assert initial is not None and "state" in initial, "initialには'state'キーを含む必要があります。"

    # 各入力のテンソルを個別のタプルに分解
    obs_inputs, context, reset_state = inputs
    ...
    # 各タイムステップでの計算
    for t in range(obs_inputs.size(1)):
        inp_t = (obs_inputs[:, t], context[:, t], reset_state[:, t])  # 各時刻の入力を取得
        out, state = cell(state, inp_t, use_obs=use_obs)  # セルを適用して出力と状態を更新
        outputs.append(out)  # 出力をリストに追加

    # リストを (B, T, ...) の形状でテンソルに変換
    outputs = torch.stack(outputs, dim=1)

    # 出力と最終ステップの状態を返す
    return outputs, {"state": {key: value[:, -1] for key, value in state["state"].items()}}
...
def _to_padded_strip(images):
    ...
    result = np.zeros(
        (
            x_dim,
            y_dim * images.shape[0] + padding * (images.shape[0] - 1),
            c_dim,
        ),  # パディングされた結果を初期化
        dtype=np.uint8,
    )
...
def compute_metrics(gt, pred):
    ...
    seq_len = gt.shape[1]
    ssim = np.zeros((batch_size, seq_len))
    psnr = np.zeros((batch_size, seq_len))
...
#             )
#             # PSNRを計算
#             psnr[i, t] = 10 * np.log10(
#                 1 / np.mean((gt[i, t] - pred[i, t]) ** 2)
#             )  # PSNRを計算
...
def plot_metrics(mean_metric, std_metric, logdir, metric_name):
    ...
    plt.figure()  # 新しい図を作成
    plt.plot(
        x, mean_metric, label=f"Mean {metric_name.upper()}"
    )  # メトリックの平均をプロット
    plt.fill_between(
        x, mean_metric - std_metric, mean_metric + std_metric, alpha=0.2
    )  # 標準偏差の範囲を塗りつぶす
    ...
    plt.savefig(
        os.path.join(logdir, f"{metric_name}.png")
    )  # メトリックのプロットを保存
    plt.close()  # プロットを閉じる
...
def save_as_grid(images, full_save_path, nrow=8):
    ...
    try:
        ...
        print(
            f"[ERROR] Failed to save image grid to {full_save_path}: {str(e)}"
        )  # エラーメッセージを表示
