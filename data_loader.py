...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        video_path = self.video_files[
            idx
        ]  # 指定されたインデックスの動画ファイルのパスを取得
        video = self._load_video(
            video_path
        )  # 動画を読み込みます（[time, height, width, channels]の形）

        # 動画からフレームを取得し、必要なシーケンス長に合わせてトリミング
        video_tensor = self._process_video(
            video
        )  # フレームを処理してテンソルに変換（[num_sequences, seq_len, channels, height, width]の形）
    ...
    def _process_video(self, video):
        ...
        # 動画をシーケンスごとに再構築
        trimmed_video = trimmed_video.reshape(
            num_sequences, self.seq_len, *trimmed_video.shape[1:]
        )  # [num_sequences, seq_len, height, width, channels]
        ...
        video_tensor = torch.stack(
            frames
        )  # [num_sequences * seq_len, channels, height, width]
        video_tensor = video_tensor.view(
            num_sequences, self.seq_len, *video_tensor.shape[1:]
        )  # [num_sequences, seq_len, channels, height, width]
...
def load_dataset(datadir, batch_size, seq_len=100, transform=transform):
    ...
    train_dataset = VideoDataset(
        video_dir=train_dir, seq_len=seq_len, transform=transform
    )
    test_dataset = VideoDataset(
        video_dir=test_dir, seq_len=seq_len, transform=transform
    )
